...
Status TraceExecutionHandler::Handle(
  ...
  fprintf(stdout, "IN TRACE RECORD HANDLER\n");
...
Status TraceExecutionHandler::Handle(
    ...
    std::unique_ptr<TraceRecordResult>* result) {
  fprintf(stdout, "IN HANDLE TRACE RECORD\n");
  if (result != nullptr) {
    result->reset(nullptr);
  }
  auto it = cf_map_.find(record.GetColumnFamilyID());
  ...
  if (!lower.empty()) {
    fprintf(stdout, "LOWER BOUND IS NOT EMPTY\n");
    r_opts.iterate_lower_bound = &lower;
  }
  Slice upper = record.GetUpperBound();
  if (!upper.empty()) {
    fprintf(stdout, "UPPER BOUND IS NOT EMPTY\n");
  ...
  switch (record.GetSeekType()) {
    ...
    default: {
      ...
      for (int j = 0; j < 10 && single_iter->Valid(); ++j) {

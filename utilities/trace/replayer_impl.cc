...
Status ReplayerImpl::ReplayTxt(
    const ReplayOptions& options,
    const std::string& txt_file,
    const std::function<void(Status, std::unique_ptr<TraceRecordResult>&&)>&
        result_callback) {

  fprintf(stdout, "IN REPLAY TEXT!\n");
  if (options.fast_forward <= 0.0) {
    return Status::InvalidArgument("Wrong fast forward speed!");
  }
  ...
  if (options.num_threads <= 1) {
    ...
    std::string line;
    uint64_t prev_access_ts = 0;
    while (std::getline(inputFile, line)) {
      std::istringstream iss(line);

      std::string key_idd;
      int type_idd, cf_idd, value_sized;
      uint64_t access_ts;

      if (iss >> key_idd >> type_idd >> cf_idd >> value_sized >> access_ts) {
        if (type_idd != 8){
          if (!cf_ids.empty()){
            std::unique_ptr<TraceRecord> record;
            record.reset(new MultiGetQueryTraceRecord(std::move(cf_ids), std::move(multiget_keys), prev_access_ts));
            cf_ids.clear();
            multiget_keys.clear();
            ...
            std::chrono::system_clock::time_point sleep_to =
                replay_epoch +
                std::chrono::microseconds(static_cast<uint64_t>(std::llround(
                    1.0 * (access_ts - header_ts_) / options.fast_forward)));
            if (sleep_to > std::chrono::system_clock::now()) {
              std::this_thread::sleep_until(sleep_to);
            }
          ...
          prev_access_ts = access_ts;
          std::unique_ptr<TraceRecord> record;
          s = TracerHelper::DecodeTraceRecordTxt(type_idd,key_idd,cf_idd,value_sized,access_ts,&record);
          if (!s.ok() && !s.IsNotSupported()) {
            break;
          }
...
//        fprintf(stdout, "line:%d ", count);
//        fprintf(stdout, "key:%s ", key_idd.substr(13,5).c_str());
          fprintf(stdout, "type_id:%d ", type_idd);
          fprintf(stdout, "cf_id:%d ", cf_idd);
          fprintf(stdout, "value_size:%d ", value_sized);
          fprintf(stdout, "ts:%lu\n ", access_ts);
        } else {
          // Multiget
          if (count > 0 && access_ts != prev_access_ts){
            std::unique_ptr<TraceRecord> record;
            record.reset(new MultiGetQueryTraceRecord(std::move(cf_ids), std::move(multiget_keys), prev_access_ts));
            cf_ids.clear();
            multiget_keys.clear();
          ...
          prev_access_ts = access_ts;
          std::string hex_key = ROCKSDB_NAMESPACE::LDBCommand::HexToString(key_idd);
          Slice tmp_key = *new Slice(hex_key);
          PinnableSlice ps;
          ps.PinSelf(tmp_key);
          multiget_keys.push_back(std::move(ps));
          cf_ids.push_back(cf_idd);
...
//          }
    ...
    if (!cf_ids.empty()){
      ...
      std::chrono::system_clock::time_point sleep_to =
          replay_epoch +
          std::chrono::microseconds(static_cast<uint64_t>(std::llround(
              1.0 * (prev_access_ts - header_ts_) / options.fast_forward)));
      if (sleep_to > std::chrono::system_clock::now()) {
        std::this_thread::sleep_until(sleep_to);
      }

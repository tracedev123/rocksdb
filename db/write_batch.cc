...
//  fprintf(stdout, "IN PUT WRITEBATCH CC\n");
  size_t ts_sz = 0;
  uint32_t cf_id = 0;
  Status s;
...
//    fprintf(stdout, "IN PUT CF WRITECC\n");
    ...
    if (recovering_log_number_ != 0) {
      ...
      if (trx) {
        ...
        if (write_after_commit_) {
          ...
          if (s.ok()) {
            ResetProtectionInfo();
            fprintf(stdout, "IN MARK COMMIT W/ TS\n");
            s = batch_info.batch_->Iterate(this);
            log_number_ref_ = 0;
          }
        }
...
//    fprintf(stdout, "IN INSERT INFO 1\n");
...
//  fprintf(stdout, "IN INSERT INFO 3\n");
...
//      fprintf(stdout, "IN UPDATE PROTECTION INFO\n");

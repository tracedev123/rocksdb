...
DEFINE_string(txt_file, "", "Txt file to be replayed. ");
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void VerifyDBFromDB(std::string& truth_db_name) {
    ...
    if (!s.ok()) {
      fprintf(stderr, "open errorAAA: %s\n", s.ToString().c_str());
      exit(1);
    }
    ReadOptions ro;
  ...
  void OpenDb(Options options, const std::string& db_name,
    ...
    if (FLAGS_num_column_families > 1) {
      ...
      for (size_t i = 0; i < num_hot; i++) {
        fprintf(stdout, "COL_FAMILI:%s \n", ColumnFamilyName(i).c_str());
...
//          "metadata", ColumnFamilyOptions(options)));
    ...
    if (!s.ok()) {
      fprintf(stderr, "open errorBBB: %s\n", s.ToString().c_str());
      exit(1);
    }
  }
...
//          fprintf(stdout, "valueHEX:%s\n", ROCKSDB_NAMESPACE::LDBCommand::StringToHex(val.ToString()).c_str());
          num_unique_keys++;
        } else {
          val = gen.Generate();
//          fprintf(stdout, "value:%s\n", val.ToString().c_str());
...
//          fprintf(stdout, "IN DOWRITE\n");
      ...
      if (!use_blob_db_) {
        // Not stacked BlobDB
//        fprintf(stdout, "STILL IN DO WRITE\n");
